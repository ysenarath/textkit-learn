...
def create_function(
    ...
    def new_func(*args: Any, **kwargs: Any) -> Any:
        ...
        default_values = [
            evaluate_ast(
                d, state, static_tools, custom_tools, authorized_imports
            )
...
def evaluate_class_def(
    ...
    bases = [
        evaluate_ast(
            base, state, static_tools, custom_tools, authorized_imports
        )
    ...
    for stmt in class_def.body:
        if isinstance(stmt, ast.FunctionDef):
            ...
            if isinstance(target, ast.Name):
                ...
                class_dict.setdefault("__annotations__", {})[target.id] = (
                    annotation
                )
...
def evaluate_call(
    ...
    if not isinstance(
        call.func,
        (ast.Call, ast.Lambda, ast.Attribute, ast.Name, ast.Subscript),
    ):
        raise InterpreterError(
    ...
    if isinstance(expression, ast.Import):
        ...
        if check_import_authorized(expression.module, authorized_imports):
            raw_module = __import__(
                expression.module,
                fromlist=[alias.name for alias in expression.names],
            )
            module = get_safe_module(raw_module, authorized_imports)
            if (
                expression.names[0].name == "*"
...
def evaluate_ast(
    ...
    if isinstance(expression, ast.Assign):
        ...
        return tuple(
            (evaluate_ast(elt, *common_params) for elt in expression.elts)
        )
...
class LocalPythonExecutor(PythonExecutor):
    ...
    def _check_authorized_imports_are_installed(self):
        ...
        missing_modules = [
            ...
            if imp != "*"
            and find_spec(base_module := imp.split(".")[0]) is None
        ]
        if missing_modules:
            raise InterpreterError(
    ...
    def __call__(self, code_action: str) -> CodeOutput:
        ...
        return CodeOutput(
            output=output, logs=logs, is_final_answer=is_final_answer
        )

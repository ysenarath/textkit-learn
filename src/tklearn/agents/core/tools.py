...
class Tool(BaseTool):
    ...
    def save(
        ...
        if make_gradio_app:
            ...
            self._write_file(
                output_path / "requirements.txt", self._get_requirements()
            )
    ...
    def from_space(
        ...
        class SpaceToolWrapper(Tool):
            ...
            def __init__(
                ...
                space_api = self.client.view_api(
                ...
                output_component = space_description_api["returns"][0][
                    "component"
                ]
            ...
            def forward(self, *args, **kwargs):
                ...
                for arg_name, arg in kwargs.items():
                    kwargs[arg_name] = self.sanitize_argument_for_prediction(
                        arg
                    )

                output = self.client.predict(
                    *args, api_name=self.api_name, **kwargs
                )
                if isinstance(output, tuple) or isinstance(output, list):
                    if isinstance(output[1], str):
                        raise ValueError(
                            "The space returned this message: " + output[1]
                        )
                    output = output[
                        0
                ...
                if isinstance(output, str) and any([
                    output.endswith(ext) for ext in IMAGE_EXTENTIONS
                ]):
                    output = AgentImage(output)
                elif isinstance(output, str) and any([
                    output.endswith(ext) for ext in AUDIO_EXTENTIONS
                ]):
    ...
    def from_gradio(gradio_tool):
        ...
        class GradioToolWrapper(Tool):
            def __init__(self, _gradio_tool):
                ...
                func_args = list(
                    inspect.signature(_gradio_tool.run).parameters.items()
                )
                self.inputs = {
                    key: {
                        "type": CONVERSION_DICT[value.annotation],
                        "description": "",
                    }
...
def launch_gradio_demo(tool: Tool):
    ...
    output_gradio_component_class = TYPE_TO_COMPONENT_CLASS_MAPPING[
        tool.output_type
    ]
    ...
    def __init__(
        ...
        **hub_kwargs,
        ...
        if not _is_package_available(
            "accelerate"
        ) or not _is_package_available("torch"):

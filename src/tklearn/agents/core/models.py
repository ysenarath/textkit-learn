            ...
            for element in message.content:
                ...
                if element["type"] == "image":
                    ...
                    if convert_images_to_image_urls:
                        ...
                        element["image"] = encode_image_base64(
                            element["image"]
                        )
...
def supports_stop_parameter(model_id: str) -> bool:
    ...
    openai_model_pattern = (
        r"(o3[-\d]*|o4-mini[-\d]*|gpt-5(-mini|-nano)?[-\d]*|gpt-5.1[-\d]*)"
    )
    grok_model_pattern = (
...
class Model:
        ...
        **kwargs,
        ...
        for kwarg_name, kwarg_value in self.kwargs.items():
            if kwarg_value is REMOVE_PARAMETER:
                ...
                completion_kwargs[kwarg_name] = (
                    kwarg_value  # Set/override parameter
                )
    ...
    def parse_tool_calls(self, message: ChatMessage) -> ChatMessage:
        ...
        assert len(message.tool_calls) > 0, (
            "No tool call was found in the model output"
        )
    ...
    def generate(
        ...
        **kwargs,
        ...
        from vllm.sampling_params import (
            StructuredOutputsParams,  # type: ignore
        )
        ...
        if stop_sequences is not None and not self.supports_stop_parameter:
            output_text = remove_content_after_stop_sequences(
                output_text, stop_sequences
            )
        return ChatMessage(
...
class MLXModel(Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.apply_chat_template_kwargs.setdefault(
            "add_generation_prompt", True
        )
        # mlx-lm doesn't support vision models: flatten_messages_as_text=True
        super().__init__(
            model_id=model_id, flatten_messages_as_text=True, **kwargs
        )
...
class TransformersModel(Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        max_new_tokens = (
            max_tokens if max_tokens is not None else max_new_tokens
        )
        ...
        **kwargs,
        ...
        stopping_criteria = (
            self.make_stopping_criteria(
                stop_sequences, tokenizer=model_tokenizer
            )
            if stop_sequences
            else None
        )
    ...
    def generate(
        ...
        **kwargs,
        ...
        if (
            ...
            and self.client_kwargs["provider"]
            not in STRUCTURED_GENERATION_PROVIDERS
        ):
            raise ValueError(
        ...
        response = self.retryer(
            self.client.chat_completion, **completion_kwargs
        )
        content = response.choices[0].message.content
        if stop_sequences is not None and not self.supports_stop_parameter:
            content = remove_content_after_stop_sequences(

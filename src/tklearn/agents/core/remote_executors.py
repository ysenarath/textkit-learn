    ...
    def _patch_final_answer_with_exception(
        ...
        def forward(self, *args, **kwargs) -> Any:
            ...
            raise FinalAnswerException(
                base64.b64encode(
                    pickle.dumps(self._forward(*args, **kwargs))
                ).decode()
            )
        ...
        _FinalAnswerTool._forward.__source__ = inspect.getsource(
            original_forward_function
        ).replace("def forward(", "def _forward(")
    ...
    def run_code_raise_errors(self, code: str) -> CodeOutput:
        ...
        if execution.error:
            ...
            if (
                ...
                == RemotePythonExecutor.FINAL_ANSWER_EXCEPTION
            ):
                final_answer = pickle.loads(
                    base64.b64decode(execution.error.value)
                )
                return CodeOutput(
                    output=final_answer,
                    logs=execution_logs,
        ...
        if not execution.results:
            return CodeOutput(
                output=None, logs=execution_logs, is_final_answer=False
            )
        ...
        for result in execution.results:
            ...
            for attribute_name in [
                ...
                if data is not None:
                    return CodeOutput(
                        output=data, logs=execution_logs, is_final_answer=False
                    )
    ...
    def cleanup(self):
        ...
        try:
            if hasattr(self, "sandbox"):
                self.logger.log(
                    "Shutting down sandbox...", level=LogLevel.INFO
                )
                self.sandbox.kill()
                self.logger.log(
                    "Sandbox cleanup completed", level=LogLevel.INFO
                )
                del self.sandbox
    ...
    try:
        ...
        while True:
            ...
            if msg_type == "stream":
                ...
                if (
                    msg_content.get("ename", "")
                    == RemotePythonExecutor.FINAL_ANSWER_EXCEPTION
                ):
                    result = pickle.loads(
                        base64.b64decode(msg_content.get("evalue", ""))
                    )
                    is_final_answer = True
                else:
                    raise AgentError(
...
def _create_kernel_http(
    crate_kernel_endpoint: str, logger, headers: Optional[dict] = None
) -> str:
...
class DockerExecutor(RemotePythonExecutor):
    ...
    def __init__(
        ...
        try:
            ...
            if build_new_image:
                self.logger.log(
                    f"Building Docker image {self.image_name}...",
                    level=LogLevel.INFO,
                )
                dockerfile_obj = BytesIO(
                    self.dockerfile_content.encode("utf-8")
                )
                _, build_logs = self.client.images.build(
                    fileobj=dockerfile_obj, tag=self.image_name
                )
            ...
            self.kernel_id = _create_kernel_http(
                f"{self.base_url}/api/kernels", self.logger
            )
            self.ws_url = (
                f"ws://{host}:{port}/api/kernels/{self.kernel_id}/channels"
            )
            ...
            self.cleanup()
            raise RuntimeError(
                f"Failed to initialize Jupyter kernel: {e}"
            ) from e
    ...
    def cleanup(self):
        ...
        try:
            if hasattr(self, "container"):
                ...
                self.logger.log(
                    "Container cleanup completed", level=LogLevel.INFO
                )
                del self.container
        except Exception as e:
            self.logger.log_error(f"Error during cleanup: {e}")
    ...
    def _wait_for_server(self):
        ...
        while not jupyter_ready and retries < 10:
            try:
                if (
                    requests.get(
                        f"{self.base_url}/api/kernelspecs", timeout=2
                    ).status_code
                    == 200
                ):
                    jupyter_ready = True
                    ...
                    self.logger.log(
                        "Jupyter not ready, waiting...", level=LogLevel.INFO
                    )
...
class ModalExecutor(RemotePythonExecutor):
    ...
    def __init__(
        ...
        if "app" not in create_kwargs:
            create_kwargs["app"] = modal.App.lookup(
                app_name, create_if_missing=True
            )

        if "encrypted_ports" not in create_kwargs:
            create_kwargs["encrypted_ports"] = [port]
        else:
            create_kwargs["encrypted_ports"] = create_kwargs[
                "encrypted_ports"
            ] + [port]
        ...
        self.logger.log(
            f"Waiting for Modal sandbox on {tunnel.host}:{port}",
            level=LogLevel.INFO,
        )
        ...
        kernel_id = _create_kernel_http(
            f"https://{tunnel.host}/api/kernels?token={token}", logger
        )
    ...
    def _wait_for_server(self, host: str, token: str):
        ...
        while True:
            try:
                ...
                if n_retries % 10 == 0:
                    self.logger.log(
                        "Waiting for server to startup, retrying...",
                        level=LogLevel.INFO,
                    )
                if n_retries > 60:
...
class BlaxelExecutor(RemotePythonExecutor):
    ...
    def __init__(
        ...
        self.sandbox_name = (
            sandbox_name or f"smolagent-executor-{uuid.uuid4().hex[:8]}"
        )
        self.image = image
        self.memory = memory
        self.region = region
        ...
        try:
            ...
            ws_scheme = (
                "wss"
                if self.sandbox.metadata.url.startswith("https")
                else "ws"
            )
            ws_base = self.sandbox.metadata.url.replace(
                "https://", ""
            ).replace("http://", "")
            ...
            self.cleanup()
            raise RuntimeError(
                f"Failed to initialize Blaxel sandbox: {e}"
            ) from e
    ...
    def install_packages(self, additional_imports: list[str]) -> list[str]:
        ...
        from blaxel.core.sandbox.client.api.process import (
            get_process_identifier,
            post_process,
        )
        from blaxel.core.sandbox.client.models import (
            ErrorResponse,
            ProcessResponse,
        )
        ...
        try:
            ...
            self.logger.log(
                f"Installing packages: {', '.join(additional_imports)}",
                level=LogLevel.INFO,
            )
            ...
            while status == "running":
                ...
                if isinstance(data, ProcessResponse):
                    ...
                    raise Exception(
                        f"Failed to install packages: {data.message}"
                    )
                else:
            ...
            if exit_code != 0:
                self.logger.log_error(
                    f"Failed to install packages (exit code {exit_code}): {logs}"
                )
                return []

            self.logger.log(
                f"Successfully installed packages: {', '.join(additional_imports)}",
                level=LogLevel.INFO,
            )
            return additional_imports
    ...
    def _delete_sandbox(self):
        ...
        self.logger.log(
            f"Requesting sandbox {self.sandbox_name} deletion...",
            level=LogLevel.INFO,
        )
    ...
    def cleanup(self):
        ...
        try:
            ...
            self.logger.log(
                f"Error during cleanup: : {e}", level=LogLevel.INFO
            )
        finally:
...
class WasmExecutor(RemotePythonExecutor):
    ...
    def __init__(
        ...
        try:
            subprocess.run(
                [deno_path, "--version"], capture_output=True, check=True
            )
    ...
    def run_code_raise_errors(self, code: str) -> CodeOutput:
        ...
        try:
            ...
            response = requests.post(
                self.server_url, json=payload, timeout=self.timeout
            )
            ...
            # Check for execution errors
                ...
                if (
                    error.get("pythonExceptionType")
                    == RemotePythonExecutor.FINAL_ANSWER_EXCEPTION
                    and "pythonExceptionValue" in error
                ):
                    result = pickle.loads(
                        base64.b64decode(error["pythonExceptionValue"])
                    )
                    is_final_answer = True
                else:
            ...
            raise AgentError(
                f"Failed to communicate with Deno server: {e}", self.logger
            )
    ...
    def install_packages(self, additional_imports: list[str]) -> list[str]:
        ...
        self.logger.log(
            f"Adding packages to load: {', '.join(additional_imports)}",
            level=LogLevel.INFO,
        )
        return additional_imports

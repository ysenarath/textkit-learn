...
class MultiStepAgent(ABC):
    ...
    def _setup_managed_agents(
        self, managed_agents: list | None = None
    ) -> None:
    ...
    def run(
        ...
        Example:
        ```py
        from tklearn.agents.core import CodeAgent
...
{str(additional_args)}."""
    ...
    def save(self, output_dir: str | Path, relative_path: str | None = None):
        ...
        with open(
            os.path.join(output_dir, "prompts.yaml"), "w", encoding="utf-8"
        ) as f:
        ...
        with open(
            os.path.join(output_dir, "agent.json"), "w", encoding="utf-8"
        ) as f:
        ...
        agent_name = (
            f"agent_{self.name}" if getattr(self, "name", None) else "agent"
        )
        ...
        with open(
            os.path.join(output_dir, "app.py"), "w", encoding="utf-8"
        ) as f:
    ...
    @classmethod
    def from_dict(
        cls, agent_dict: dict[str, Any], **kwargs
    ) -> "MultiStepAgent":
        ...
        model_class = getattr(
            importlib.import_module("tklearn.agents.core.models"),
            model_info["class"],
        )
        ...
        for managed_agent_dict in agent_dict["managed_agents"]:
            agent_class = getattr(
                importlib.import_module("tklearn.agents.core.agents"),
                managed_agent_dict["class"],
            )
    ...
    def from_hub(
        ...
        **kwargs,
        ...
        download_folder = Path(
            snapshot_download(repo_id=repo_id, **download_kwargs)
        )
    ...
    def from_folder(cls, folder: str | Path, **kwargs):
        ...
        for managed_agent_name, managed_agent_class_name in agent_dict[
            ...
            agent_cls = getattr(
                importlib.import_module("tklearn.agents.core.agents"),
                managed_agent_class_name,
            )
            managed_agents.append(
                agent_cls.from_folder(
                    folder / "managed_agents" / managed_agent_name
                )
            )
        agent_dict["managed_agents"] = {}
...
class ToolCallingAgent(MultiStepAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        prompt_templates = prompt_templates or yaml.safe_load(
            importlib.resources.files("tklearn.agents.core.prompts")
    ...
    def _step_stream(
        ...
        if (
            chat_message.tool_calls is None
            or len(chat_message.tool_calls) == 0
        ):
            try:
    ...
    def process_tool_calls(
        ...
        def process_single_tool_call(tool_call: ToolCall) -> ToolOutput:
            ...
            tool_call_result = self.execute_tool_call(
        ...
        if len(parallel_calls) == 1:
            ...
            with ThreadPoolExecutor(self.max_tool_threads) as executor:
                ...
                for tool_call in parallel_calls.values():
                    ...
                    futures.append(
                        executor.submit(
                            ctx.run, process_single_tool_call, tool_call
                        )
                    )
...
class CodeAgent(MultiStepAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._use_structured_outputs_internally = (
        ...
        if (
            isinstance(code_block_tags, str)
            and not code_block_tags == "markdown"
        ):
            raise ValueError(
    ...
    def _step_stream(
        ...
        try:
            if self._use_structured_outputs_internally:
                ...
                code_action = parse_code_blobs(
                    output_text, self.code_block_tags
                )

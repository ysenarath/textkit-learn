...
class DuckDuckGoSearchTool(Tool):
    ...
    Examples:
        ```python
        >>> from tklearn.agents.core import DuckDuckGoSearchTool
    ...
    def __init__(
        self, max_results: int = 10, rate_limit: float | None = 1.0, **kwargs
    ):
        super().__init__()
...
class ApiWebSearchTool(Tool):
    ...
    Examples:
        ```python
        >>> from tklearn.agents.core import ApiWebSearchTool
...
class WikipediaSearchTool(Tool):
    ...
    Example:
        ```python
        >>> from tklearn.agents.core import CodeAgent, InferenceClientModel, WikipediaSearchTool
        >>> agent = CodeAgent(
        >>>     tools=[
        >>>            WikipediaSearchTool(
    ...
    def __init__(
        self,
        user_agent: str = "tklearn (myemail@example.com)",
        language: str = "en",
    ...
    def forward(self, query: str) -> str:
        try:
            ...
            if self.content_type == "summary":
                ...
                return (
                    "⚠️ Invalid `content_type`. Use either 'summary' or 'text'."
                )

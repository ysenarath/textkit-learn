...
class AgentImage(AgentType, PIL.Image.Image):
    ...
    def __init__(self, value):
        ...
        if isinstance(value, AgentImage):
            self._raw, self._path, self._tensor = (
                value._raw,
                value._path,
                value._tensor,
            )
...
class AgentAudio(AgentType, str):
    ...
    def __init__(self, value, samplerate=16_000):
        if not _is_package_available("soundfile") or not _is_package_available(
            "torch"
        ):
...
def handle_agent_input_types(*args, **kwargs):
    args = [
        (arg.to_raw() if isinstance(arg, AgentType) else arg) for arg in args
    ]
    kwargs = {
        k: (v.to_raw() if isinstance(v, AgentType) else v)
...
def handle_agent_output_types(
    output: Any, output_type: str | None = None
) -> Any:

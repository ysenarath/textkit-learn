...
ALL_COLOR = [getattr(Fore, x) for x in ALL_COLOR_NAME] + [
    getattr(Back, x) for x in ALL_COLOR_NAME
]
...
def maybe_convert_to_colorama_str(color: str) -> str:
    ...
    assert color in ALL_COLOR_STYLE, (
        f"Color {color!r} incorrect! Available: {' '.join(ALL_COLOR_STYLE_NAME)}"
    )
    return color
    ...
    if isinstance(description, str):
        for obj in available_pbar_styles():
            if _contains_word(obj.name, description):
                ...
                color, color_empty, description = (
                    _parse_colors_from_description(description)
                )
                is_alt = "alt" in description
                is_clean = "clean" in description
                description = (
                    description.replace("alt", "")
                    .replace("clean", "")
                    .strip(" ")
                )
        ...
        available_names = ", ".join([
            obj.name for obj in available_pbar_styles()
        ])
    ...
    if isinstance(description, str):
        for obj in available_table_styles():
            if _contains_word(obj.name, description):
                ...
                return obj()
        available_names = ", ".join([
            obj.name for obj in available_table_styles()
        ])
...
def available_table_styles() -> list[type[TableStyleBase]]:
    ...
    return [
        ...
        and issubclass(obj, TableStyleBase)
...
def available_pbar_styles() -> list[type[PbarStyleBase]]:
    ...
    return [
        ...
        for name, obj in globals().items()
        if isinstance(obj, type)
        and issubclass(obj, PbarStyleBase)
        and hasattr(obj, "name")
    ]
...
def get_aggregate_fn(aggregate: None | str | Callable) -> Callable:
    ...
    if callable(aggregate):
        ...
        assert num_parameters == 4, (
            f"Aggregate function has to take 4 arguments, not {num_parameters}!"
        )
        return aggregate
...
class ProgressTable:
    ...
    def __init__(
        ...
        *cols: str,
        ...
        pbar_style: str | PbarStyleBase = "square",
        pbar_style_embed: str | PbarStyleBase = "cdots",
        print_header_on_top: bool = True,
        print_header_every_n_rows: int = 30,
        custom_cell_format: Callable[[Any], str] | None = None,
        table_style: str | TableStyleBase = "round",
        ...
        if custom_format is not None:
            logger.warning(
                "Argument `custom_format` is deprecated. Use `custom_cell_format` instead!"
            )
        if embedded_progress_bar is not None:
            logger.warning(
                "Argument `embedded_progress_bar` is deprecated. Use `pbar_embedded` instead!"
            )
        if print_row_on_update is not None:
            logger.warning(
                "Argument `print_row_on_update` is deprecated. Specify `interactive` instead!"
            )

        self.pbar_style = parse_pbar_style(pbar_style)
        self.table_style = parse_table_style(table_style)
        self.pbar_style_embed = parse_pbar_style(pbar_style_embed)

        assert isinstance(default_row_color, ColorFormatTuple), (
            "Row color has to be a color format!"
        )
        assert isinstance(default_column_color, ColorFormatTuple), (
            "Column color has to be a color format!"
        )
        assert isinstance(default_header_color, ColorFormatTuple), (
            "Header color has to be a color format!"
        )
        ...
        self.column_names: list[
            str
        ] = []  # Names serve as keys for column configs
        self._closed = False
        ...
        self.custom_cell_format = custom_cell_format or get_default_format_fn(
            num_decimal_places
        )
        ...
        self._frame_time: float = (
            1 / self.refresh_rate if self.refresh_rate else 0.0
        )
    ...
    def add_column(
        ...
        *,
        ...
        assert isinstance(name, str), (
            f"Column name has to be a string, not {type(name)}!"
        )
        ...
        resolved_width = (
            width or self.column_width or self.DEFAULT_COLUMN_WIDTH
        )
        if not width and resolved_width < len(str(name)):
            resolved_width = len(str(name))
        self.column_widths[name] = resolved_width
        self.column_colors[name] = maybe_convert_to_colorama(
            color or self.column_color or self.DEFAULT_COLUMN_COLOR
        )
        self.column_alignments[name] = (
            alignment or self.column_alignment or self.DEFAULT_COLUMN_ALIGNMENT
        )
        self.column_aggregates[name] = get_aggregate_fn(
            aggregate
            or self.column_aggregate
            or self.DEFAULT_COLUMN_AGGREGATE,
        )
    ...
    def reorder_columns(self, *column_names) -> None:
        ...
        assert all(x in self.column_names for x in column_names), (
            f"Columns {column_names} not in {self.column_names}"
        )
        ...
        self.column_alignments = {
            k: self.column_alignments[k] for k in column_names
        }
        self.column_aggregates = {
            k: self.column_aggregates[k] for k in column_names
        }
        self._set_all_display_rows_as_pending()
    ...
    def update(
        ...
        **column_kwds,
        ...
        data_row.values[name] = fn(
            value, data_row.values[name], weight, data_row.weights[name]
        )
    ...
    def __setitem__(self, key: str | tuple[str, int], value: Any) -> None:
        ...
        assert isinstance(row, int), (
            f"Row {row} has to be an integer, not {type(row)}!"
        )
    ...
    def __getitem__(self, key: str | tuple[str, int]) -> Any:
        ...
        if isinstance(key, tuple):
            ...
            name = key
            row = -1
        assert name in self.column_names, (
            f"Column {name} not in {self.column_names}"
        )
        assert isinstance(row, int), (
            f"Row {row} has to be an integer, not {type(row)}!"
        )
    ...
    def next_row(
        ...
        if (
            header is None
            and len(self._data_rows) - self._previous_header_row_number
            >= self._print_header_every_n_rows
        ...
        if header:
            ...
            self._latest_row_decorations.extend([
                "SPLIT MID",
                "HEADER",
                "SPLIT MID",
            ])
    ...
    def to_list(self) -> list[list[Any]]:
        ...
        values = [
            [row.values.get(col, None) for col in self.column_names]
    ...
    def to_df(self) -> Any:
        ...
        return pd.DataFrame(
            self.to_list(), columns=pd.Series(self.column_names)
        )
    ...
    def _get_item_str(self, display_row_index: int) -> str:
        ...
        if isinstance(item, int):
            ...
            row_str = self._get_row_str(
                row
            )  # here we pass the row item, not index
        elif item == "HEADER":
    ...
    def _print_pending_rows_to_buffer(self) -> None:
        ...
        for display_row_idx, cleaning_str in self._cleaning_pbar_instructions:
            assert self.interactive >= 2, (
                "Should not need to clean pbars when interactive < 2!"
            )
        ...
        for pbar in self._active_pbars:
            ...
            pbar_display_row_idx = (
                self._display_rows.index(pbar.position)
                if pbar.static
                else num_rows + pbar.position - 1
            )
            ...
            if pbar_display_row_idx > num_rows:
                self._cleaning_pbar_instructions.append((
                    pbar_display_row_idx,
                    pbar._cleaning_str,
                ))
    ...
    def _resolve_row_color_dict(
        self, color: ColorFormat | dict[str, ColorFormat] = None
    ) -> dict[str, str]:
        ...
        color = {
            column: color.get(column) or self.DEFAULT_ROW_COLOR
            for column in self.column_names
        }
        color_colorama = {
            column: maybe_convert_to_colorama(color)
            for column, color in color.items()
        }
        return {
            col: self.column_colors[col] + color_colorama[col] for col in color
        }

    def _apply_cell_formatting(
        self, value: Any, column_name: str, color: str
    ) -> str:
    ...
    def _get_outer_inner_width(self) -> int:
        return (
            sum(self.column_widths.values()) + 3 * len(self.column_widths) + 1
        )
    ...
    def _print_to_buffer(
        self, msg: str = "", prefix: str = "", suffix: str = ""
    ) -> None:
    ...
    def _get_row_str(self, row: DataRow, *, colored: bool = True) -> str:
        ...
        for column in self.column_names:
            ...
            value = self._apply_cell_formatting(
                value=value, column_name=column, color=color
            )
    ...
    def _get_bar(self, left: str, center: str, right: str) -> str:
        ...
        for column_name in self.column_names:
            content_list.append(
                self.table_style.horizontal
                * (self.column_widths[column_name] + 2)
            )
    ...
    def _get_bar_mid(self) -> str:
        return self._get_bar(
            self.table_style.no_left,
            self.table_style.all,
            self.table_style.no_right,
        )
    ...
    def _get_header(self) -> str:
        ...
        colors = (
            ...
            else self._resolve_row_color_dict(self.header_color)
        )

        for column in self.column_names:
            value = self._apply_cell_formatting(
                column, column, color=colors[column]
            )
            content.append(value)
        return "".join(
            [
        ...
        *range_args,
        ...
        if position is None:
            position = (
                ...
                else len(self._active_pbars) + 1 - self.pbar_embedded
            )

        total = (
            total
            if total is not None
            else (len(iterable) if isinstance(iterable, Sized) else 0)
        )

        style = parse_pbar_style(style) if style else self.pbar_style
        style_embed = (
            parse_pbar_style(style_embed)
            if style_embed
            else self.pbar_style_embed
        )
        ...
        pbar = TableProgressBar(
            ...
            show_throughput=(
                show_throughput
                if show_throughput is not None
                else self.pbar_show_throughput
            ),
            show_progress=(
                show_progress
                if show_progress is not None
                else self.pbar_show_progress
            ),
            show_percents=(
                show_percents
                if show_percents is not None
                else self.pbar_show_percents
            ),
...
class TableAtIndexer:
    ...
    def __init__(self, table: ProgressTable) -> None:
        ...
        for word in ("values", "weights", "colors"):
            self.edit_mode_prefix_map.update({
                word[:i].lower(): word for i in range(1, len(word) + 1)
            })
            self.edit_mode_prefix_map.update({
                word[:i].upper(): word for i in range(1, len(word) + 1)
            })
    ...
    def _parse_index(self, key: slice | tuple) -> tuple:
        if isinstance(key, slice):
            ...
            assert mode in self.edit_mode_prefix_map, (
                f"Unknown mode `{mode}`. Available: values, weights, colors"
            )
        ...
        assert isinstance(rows, (slice, int)), (
            f"Rows have to be a slice or an integer, not {type(rows)}!"
        )
        assert isinstance(cols, (slice, int)), (
            f"Columns have to be a slice or an integer, not {type(cols)}!"
        )
        row_indices = (
            [rows]
            if isinstance(rows, int)
            else list(range(len(self.table._data_rows)))[rows]
        )
        column_names = (
            self.table.column_names[cols]
            if isinstance(cols, slice)
            else [self.table.column_names[cols]]
        )
    ...
    def __setitem__(self, key: slice | tuple, value: Any) -> None:
        ...
        if edit_mode == "colors":
            assert isinstance(value, ColorFormatTuple), (
                f"Color must be compatible with ColorFormat, not {type(value)}!"
            )
    ...
    def __getitem__(self, key: slice | tuple) -> list:
        ...
        for row_idx in row_indices:
            ...
            row_values = [
                row.__getattribute__(edit_mode).get(c, None)

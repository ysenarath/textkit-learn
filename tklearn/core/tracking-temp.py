...
def explore_recursively(
    ...
    for file in os.listdir(run_path):
        ...
        if d["type"] != "parameter":
            continue
        d["metadata.base_run.id"] = base_run_id
    ...
    for file in os.listdir(run_path):
        ...
        if file == "metadata.json":
            continue
        if subpath.is_dir() and file.startswith("run-"):
        ...
        d["parameters"] = [
            ...
            for p in parameters
            if p["step"] is None
            or (p["step"] == d["step"] and d["metadata.run.id"] == p["metadata.run.id"])
        ]
        data.append(d)
    return data
...
class Run(object):
    ...
    def get_logs(self) -> dict:
        return explore_recursively(self.run_path)
...
class StatType:
    ...
    def __repr__(self):
        return "{:0.2f}±{:0.2f}".format(self.mean, self.std)


@pd.api.extensions.register_dataframe_accessor("stat")
class AggAccessor:
    def __init__(self, df: pd.DataFrame):
        self._obj: Union[pd.DataFrame, pd.Series] = (
            df[df["type"] == "metric"]
            .pivot_table(
                values="value",
                index="step",
    ...
    def __getitem__(self, name):
        new = AggAccessor.__new__(AggAccessor)
        new._obj = self._obj[name]
        return new
    ...
    @property
    def index(self):
        # get the means of the dataframe/series
        return self._obj.index

    @property
    def mean(self):
        # get the means of the dataframe/series
        return self._obj.apply(lambda x: x.mean)

    @property
    def std(self):
        # get the std of the dataframe/series
        return self._obj.apply(lambda x: x.std)

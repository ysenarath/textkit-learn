...
def get_context_window(
    sentence: str, target_word: str, window_size: int = 3
) -> List[str]:
...
def lesk_wsd(
    sentence: str, target_word: str, pos: str | None = None
) -> Synset | None:
    # Get context window around target word
    context_words = get_context_window(sentence, target_word)
    ...
    for sense in synsets:
        ...
        score = compute_overlap(context_words, sense_words)
        if score > best_score:
            best_score = score
            best_sense = sense
    return best_sense
...
def disambiguate_sentence(sentence: str) -> List[Tuple[str, Synset]]:
    ...
    for word, tag in tagged:
        ...
        if pos is None:
            continue
        sense = lesk_wsd(sentence, word, pos)
